...
```mermaid
graph TD
    A["AI 智能体"] -->|"MCP 请求"| B["Cursor IDE"]
    B -->|"触发"| C["Review Gate 弹窗"]
    C -->|"用户输入"| D{"输入类型"}
    D -->|"文本"| E["文本处理"]
    D -->|"语音"| F["语音转文字"]
    D -->|"图片"| G["图片上传"]
...
```mermaid
graph TD
    A[下载项目] --> B[备份 mcp.json]
    B --> C[关闭 Cursor MCP]
    C --> D[以管理员身份运行 install.bat]
    D --> E{Python 检查}
    E -->|"✓ Python 3.10+"| F[创建虚拟环境]
    E -->|"✗ 缺失"| G[安装 Python]
    G --> F
    F --> H[安装依赖包]
    H --> I[配置 MCP 服务器]
    I --> J[安装 Cursor 扩展]
    J --> K[复制规则到 Cursor 规则目录]
    K --> L[重启 Cursor IDE]
    L --> M{测试安装}
    M -->|"✓ 成功"| N[准备使用]
    M -->|"✗ 失败"| O[检查日志排除故障]
...
2. **运行安装脚本**：
   ...
   * 进入项目根目录，此时建议关闭cursor中的所有mcp,并备份。
   * **双击运行 `install.bat` 文件,强烈推荐右键`用管理员身份`运行该程序。**。
